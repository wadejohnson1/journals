 ...
 * particular, it does not restore the current play state, play position or selected tracks.
 * Applications should save and restore these on their own in {@link
 * android.app.Activity#onSaveInstanceState} and {@link android.app.Activity#onRestoreInstanceState}.
 * <p/>
...
public class VideoView extends SurfaceView implements MediaPlayerControl {
    ...
    /**
     * Media player internal state.
     */
    private static final int STATE_ERROR = -1, STATE_IDLE = 0, STATE_PREPARING = 1,
            STATE_PREPARED = 2, STATE_PLAYING = 3, STATE_PAUSED = 4, STATE_PLAYBACK_COMPLETED = 5;
    /**
     * Tag for displaying debug messages.
     */
    private static final String DEBUG_TAG = VideoView.class.getSimpleName();

    /**
     * Video view object's current state.
     */
    private int mCurrentState = STATE_IDLE;
    /**
     * State that a method caller intends to reach.
     * <p/>
     * For instance, regardless the VideoView object's current state, calling pause() intends to
     * bring the object to a target state of {@link #STATE_PAUSED}.
     */
    ...
    private MediaActivity mMediaController;
    ...
    private void openVideo() {
        ...
        try {
            ...
            Log.w(DEBUG_TAG, "Unable to open content: " + mUri, ex);
    ...
    public void setMediaController(MediaActivity controller) {
        if (mMediaController != null) {
            mMediaController.hideDecorViews();
        }
            ...
            if (mVideoWidth != 0 && mVideoHeight != 0) {
                ...
                if (mSurfaceWidth == mVideoWidth && mSurfaceHeight == mVideoHeight) {
                    ...
                    if (mTargetState == STATE_PLAYING) {
                        start();
                        if (mMediaController != null) {
                            mMediaController.showDecorViews(MediaActivity.DEFAULT_TIMEOUT);
                        }
                    } else if (!isPlaying() &&
                            (seekToPosition != 0 || getCurrentPosition() > 0)) {
                        if (mMediaController != null) {
                            // Show the media controls when we're paused into a video and make 'em stick.
                            mMediaController.showDecorViews(0);
                        }
                    }
                }
            } else {
    ...
    private MediaPlayer.OnErrorListener mErrorListener =
            new MediaPlayer.OnErrorListener() {
                public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
                    Log.d(DEBUG_TAG, "Error: " + framework_err + "," + impl_err);
                    mCurrentState = STATE_ERROR;
                    mTargetState = STATE_ERROR;
                    if (mMediaController != null) {
                        mMediaController.hideDecorViews();
             ...
             */
                    if (getWindowToken() != null) {
                        ...
                        if (framework_err ==
                            ...
                            messageId =
                                    R.string.dialog_message_error_video_invalid_progressive_playback;
                        } else {
                            messageId = R.string.dialog_message_error_video_unknown;
                        }

                        new AlertDialog.Builder(getContext())
                                .setMessage(messageId)
                                .setPositiveButton(
                                        R.string.dialog_positiveButton_ok,
                                        new DialogInterface.OnClickListener() {
                                            public void onClick(DialogInterface dialog,
                                                                int whichButton) {
                                        /* If we get here, there is no onIOError listener, so
    ...
    SurfaceHolder.Callback mSHCallback = new SurfaceHolder.Callback() {
        ...
        public void surfaceDestroyed(SurfaceHolder holder) {
            ...
            mSurfaceHolder = null;
            if (mMediaController != null) mMediaController.hideDecorViews();
            release(true);
        }
    };
